...
## Dev Benefits
The screenshot here shows an existing tool called Debugger Canvas from Microsoft. The tool uses an infinite 2D canvas to visualize the flow of a program through methods and the callstack as you debug it. A similar tool in VR could take advantage of an unlimited 3D space where the visualization surrounds you, so that you could always have that spatial context.
